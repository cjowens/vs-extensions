...
const coauthoringServiceTypes_1 = require("../../workspace/contract/coauthoringServiceTypes");
...
class VSCodeBufferManager {
    ...
    undoLastLocalEdit() {
        ...
        util_1.assert(this.localToRemoteTransitions.length > 0, 'No local changes, but no transitions');
        ...
        this.host.undoBufferToMatchContents(targetBufferState.beforeTransitionToRemote).then(() => {
            this.host.trace.verbose('Reached target buffer state through editor undos');
            ...
            util_1.assert(this.localToRemoteTransitions.length === originalLocalToRemoteTransitions, 'Didn\'t expect transitions while we were undoing');
            ...
            // keep undoing themselves when we need attempt an undo
            if (previousStateTransition && previousStateTransition.beforeTransitionToLocal !== content) {
                ...
                // IDE created an undo stop in during the customers), so if the
    ...
    redoLastLocalEdit() {
        ...
        util_1.assert(this.localRedoStack.length > 0, 'We don\'t have any items to redo');
    ...
    makeSnapshot() {
        ...
        return {
            ...
            hashCode: util_2.calculateFileHash(currentContent),
    ...
    processQueuedMessages() {
        ...
        if (this.diagnosticTrace) {
            util_1.logger.logTrace(this.host.trace, 'step');
            util_1.logger.indent();
            util_1.logger.logTrace(this.host.trace, `before local changes - collabbuffer: ` + util_1.colorize.cyan(JSON.stringify(this.collabBuffer.getContent())));
        }
        ...
        if (this.diagnosticTrace) {
            util_1.logger.logTrace(this.host.trace, `after localchanges - collabbuffer: ` + util_1.colorize.cyan(JSON.stringify(this.collabBuffer.getContent())));
        }
        ...
        while (this.remoteMessagesQueue.length > 0 && continueProcessing) {
            ...
            switch (msgType) {
                case coauthoringServiceTypes_1.MessageType.TextChange:
                    ...
                    continue;
                case coauthoringServiceTypes_1.MessageType.SelectionChange:
                ...
                default:
                    ...
                    util_1.assert(false, errMsg);
                    break;
            }
        }
        if (this.diagnosticTrace) {
            util_1.logger.unindent();
        }
    }
    ...
    _handleSelectionChangeMessageIfMostRecent(forClientId, withMessage) {
        ...
        for (let i = 1; i < this.remoteMessagesQueue.length; ++i) {
            ...
            if (msg.clientId === forClientId) {
                foundSubsequentSelectionChange = msg.messageType === coauthoringServiceTypes_1.MessageType.SelectionChange;
                break;
            }
        }
    ...
    _beginApplyingRemoteChange(message, serverVersionNumber) {
        ...
        if (this.lastChangeSource !== LastChangeSource.Remote) {
            util_1.assert(!this.performingUndo, 'Shouldn\'t see remote changes while undoing');
            this.localToRemoteTransitions.push({ beforeTransitionToRemote: this.collabBuffer.getContent(), hadLocalEdits: this.hasEverHadALocalEdit });
        }
        ...
        if (this.diagnosticTrace) {
            util_1.logger.logTrace(this.host.trace, `sending edit request to ext host: ` + JSON.stringify(edits));
            util_1.logger.logTrace(this.host.trace, `sending edit request to ext host: ` + JSON.stringify(vscodeEdits));
        }
        ...
        this.host.applyEdit(vscodeEdits).then((applied) => {
            if (!applied) {
                this._finishApplyingRemoteChangeFail(originalLastChangeSource);
                return;
            }
    ...
    _mapTextChangeToEditorEdit(change) {
        ...
        if (isReplacingSingleLF && newTextEndsWithLF) {
            // We're only adding text, but the collab buffer has automatically included the adjacent LF into the
    ...
    _finishApplyingRemoteChangeOK() {
        ...
        util_1.assert(this.localChangesQueue.length === 1, `expected a single local change caused by my own edit, but got ${JSON.stringify(this.localChangesQueue)}`);
    ...
    _finishApplyingRemoteChangeFail(originalLastChangeSource) {
        this.host.trace.info('Applying edits to main buffer failed');
        let pendingEdit = this.pendingApplicationOfRemoteEdit;
        this.pendingApplicationOfRemoteEdit = null;
