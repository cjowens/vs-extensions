...
class CollabBuffer {
     ...
     */
    getLineContent(line) {
        util_1.assert(line < this.lines.length, 'Line content requested outside of buffer');
    ...
    static _sortAndValidateEdits(maxPosition, edits) {
        ...
        edits.forEach((edit) => {
            util_1.assert(edit.offset >= 0, 'invalid position');
            util_1.assert(edit.offset <= maxPosition, 'invalid position');
            util_1.assert(edit.length >= 0, 'invalid length');
            util_1.assert(edit.offset + edit.length <= maxPosition, 'invalid length');
        });
        // Sort edits
        ...
        edits.forEach((currentEdit, index, source) => {
            ...
            if (previousEdit.offset + previousEdit.length > currentEdit.offset) {
                util_1.assert(false, 'invalid edits: overlapping');
            }
        });
        return edits;
    }
