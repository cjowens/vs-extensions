...
class OTAlgorithm {
    ...
    acceptLocalChange(originalChanges, unexpandedChanges) {
        if (this.diagnosticTrace) {
            util_1.logger.log(`acceptOriginalBufferChange: ${JSON.stringify(originalChanges)}`);
    ...
    acceptRemoteChange(buffer, message, eventId) {
        if (this.diagnosticTrace) {
            util_1.logger.log(`acceptRemoteChange: ${JSON.stringify(message)}`);
            util_1.logger.indent();
        }
        util_1.assert(message.clientId !== this.host.clientID, 'Changes must be from a remote client');
        // Undo our unacknowledged changes
        if (this.diagnosticTrace) {
            util_1.logger.log(`beginning to undo unacknowledged changes.`);
            util_1.logger.indent();
        }
        ...
        if (this.diagnosticTrace) {
            util_1.logger.unindent();
            util_1.logger.log(`finished undo of unacknowledged changes.`);
            util_1.logger.log(`beginning to apply server message.`);
            util_1.logger.indent();
        }
        ...
        if (this.diagnosticTrace) {
            util_1.logger.unindent();
            util_1.logger.log(`finished applying server message.`);
            util_1.logger.log(`beginning to redo unacknowledged changes.`);
            util_1.logger.indent();
        }
        for (let i = 0; (i < this.unacknowledgedChanges.length); ++i) {
        ...
        if (this.diagnosticTrace) {
            util_1.logger.unindent();
            util_1.logger.log(`finished redo of unacknowledged changes.`);
            util_1.logger.unindent();
        }
    }
    ...
    _applyOperationOnBuffer(buffer, message) {
        ...
        util_1.assert(message.changeServerVersion === this.transformManager.CurrentServerVersionNumber, 'Cannot apply change on buffer due to different contexts.');
        ...
        for (let changeIndex = 0; (changeIndex < message.changes.length); ++changeIndex) {
            ...
            // Here we eliminate overlapping edits
            {
                ...
                if (newStart !== start || newEnd !== end) {
                    if (this.diagnosticTrace) {
                        util_1.logger.log(util_1.colorize.red(`warning`) + `: overlapping edits: position: ${start}, length: ${end - start} has been clamped to position: ${newStart}, length: ${newEnd - newStart}`);
                    }
                    start = newStart;
                    end = newEnd;
                }
            ...
            // Here we clamp edits which are out of bounds
            {
                ...
                if (newStart !== start || newEnd !== end) {
                    if (this.diagnosticTrace) {
                        util_1.logger.log(util_1.colorize.red(`warning`) + `: edit out of bounds: position: ${start}, length: ${end - start} has been clamped to position: ${newStart}, length: ${newEnd - newStart}`);
                    }
                    start = newStart;
                    end = newEnd;
                }
    ...
    acceptTextChangeAcknowledge(message, eventId) {
        if (this.diagnosticTrace) {
            util_1.logger.log(`acceptTextChangeAcknowledge`);
            util_1.logger.indent();
        }
        util_1.assert(message.clientId === this.host.clientID, 'Changes must a change we initiated');
