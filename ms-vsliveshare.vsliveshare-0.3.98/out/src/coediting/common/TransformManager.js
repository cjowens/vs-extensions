...
class TransformManager {
    ...
    RollbackTo(serverVersionNumber) {
        ...
        util_1.assert(false, 'Failed to find version number to remove');
    }
    ...
    TransformMessageToVersionWithMessageIndex(message, targetVersion, messageIndex) {
        util_1.assert(message !== null, 'message must not be null');
        util_1.assert(targetVersion >= message.changeServerVersion, 'Can\'t translate messages to prior than their creation');
        ...
        for (let serverVersionIndex = maxServerVersionIndex; (serverVersionIndex >= serverVersionStartIndex); --serverVersionIndex) {
            ...
            if (serverVersion.Message.clientId === message.clientId) {
                ...
                for (let changeIndex = 0; (changeIndex < copyOfChanges.length); ++changeIndex) {
                    ...
                    util_1.assert(start >= 0, 'start must be positive');
                    util_1.assert(end >= start, 'end must not come before start');
                    change.start = start;
    ...
    AddMessage(message) {
        for (let i = this.messages.length - 1; (i >= 0); --i) {
            ...
            if (message.changeServerVersion >= existingMessage.changeServerVersion) {
                util_1.assert(message.changeServerVersion !== existingMessage.changeServerVersion, 'Should never try to insert a message we have');
                util_1.assert(message.clientId === existingMessage.clientId, 'Client IDs should never change');
                this.messages.splice(i + 1, 0, message);
                return;
            }
        }
        util_1.assert(false, 'Never should try to insert a new version before index 0');
    }
}
