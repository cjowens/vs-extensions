...
function toUri(fileName) {
    return fileName.startsWith('vsls:') ? vscode.Uri.parse(fileName) : vscode.Uri.file(fileName);
}
exports.toUri = toUri;
function toLocation(location) {
    const fileName = toUri(location.FileName);
...
function getSchemeQualifiedFileName(document) {
    switch (document.uri.scheme) {
        ...
        case "omnisharp-metadata":
            return `${document.uri.authority}${document.fileName.replace("[metadata] ", "")}`;
        case "vsls":
            // The scheme qualified filename is always expected to have forward slashes like the uri. 
            return document.uri.toString();
        default:
            return document.fileName;
    }
}
exports.getSchemeQualifiedFileName = getSchemeQualifiedFileName;
function createRequest(document, where, includeBuffer = false) {
    ...
    let request = {
        FileName: getSchemeQualifiedFileName(document),
