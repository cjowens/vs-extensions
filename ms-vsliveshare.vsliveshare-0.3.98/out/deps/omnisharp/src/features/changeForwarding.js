...
function forwardDocumentChanges(server) {
    return vscode_1.workspace.onDidChangeTextDocument(event => {
        let { document } = event;
        if (document.uri.scheme !== "vsls" || document.languageId !== 'csharp') {
            return;
        }
        if (!server.isRunning()) {
            return;
        }
        serverUtils.updateBuffer(server, { Buffer: document.getText(), FileName: typeConvertion_1.getSchemeQualifiedFileName(document) }).catch(err => {
            console.error(err);
            return err;
        });
    });
