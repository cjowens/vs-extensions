...
class OmniSharpCodeLensProvider extends abstractProvider_1.default {
    ...
    provideCodeLenses(document, token) {
        ...
        return serverUtils.currentFileMembersAsTree(this._server, { FileName: typeConvertion_1.getSchemeQualifiedFileName(document) }, token).then(tree => {
            let ret = [];
            tree.TopLevelTypeDefinitions.forEach(node => this._convertQuickFix(ret, typeConvertion_1.getSchemeQualifiedFileName(document), node));
            return ret;
        });
    }
        ...
        if (codeLens instanceof OmniSharpCodeLens) {
            ...
            return serverUtils.findUsages(this._server, req, token).then(res => {
                ...
                codeLens.command = {
                    ...
                    arguments: [typeConvertion_1.toUri(req.FileName), codeLens.range.start, res.QuickFixes.map(typeConvertion_1.toLocation)]
                };
                return codeLens;
            });
        }
