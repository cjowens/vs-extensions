...
const remoteReadableStream_1 = require("./remoteReadableStream");
...
class TypeScriptServiceClient {
    constructor(host, workspaceState, onDidChangeTypeScriptVersion, plugins, remoteManager) {
        this.host = host;
        ...
        this.disposables = [];
        this.remoteCancellationRequested = false;
    ...
    startService(resendModels = false) {
        ...
        return this.servicePromise = new Promise((resolve, reject) => {
            ...
            try {
                if (this.remoteManager) {
                    this.info('Using remote TSServer');
                    this.lastStart = Date.now();
                    // tslint:disable-next-line:no-unused-expression
                    new wireProtocol_1.Reader(new remoteReadableStream_1.RemoteReadableStream(this.remoteManager), (msg) => { this.dispatchMessage(msg); }, error => { this.error('ReaderError', error); });
                    this._onReady.resolve();
                    resolve(undefined);
                    this._onTsServerStarted.fire();
                    setTimeout(() => { this.serviceStarted(resendModels); }, 0);
                }
                else {
                    const options = {
    ...
    normalizePath(resource) {
        ...
        if (resource.scheme !== fileSchemes.file && resource.scheme !== 'vsls') {
            return null;
        }
    ...
    asUrl(filepath) {
        ...
        return convert_1.toUri(filepath);
    }
    ...
    sendNextRequests() {
        while (this.callbacks.pendingResponses === 0 && this.requestQueue.length > 0) {
            if (this.remoteCancellationRequested) {
                this.remoteManager.writeLineAsync("typescript_ls_cancellation" /* TypeScriptLanguageServiceCancellation */, 'clean');
                this.remoteCancellationRequested = false;
            }
    ...
    sendRequest(requestItem) {
        ...
        const request = JSON.stringify(serverRequest);
        this.service()
            .then((childProcess) => {
            this.remoteManager.writeLineAsync("typescript_ls" /* TypeScriptLanguageService */, request).then((isSatisfiedRemotely) => {
                if (!isSatisfiedRemotely) {
            ...
            if (this.apiVersion.has222Features() && this.cancellationPipeName) {
                ...
                try {
                    if (this.remoteManager.isRemoteSession()) {
                        this.remoteManager.writeLineAsync("typescript_ls_cancellation" /* TypeScriptLanguageServiceCancellation */, 'cancel');
                        this.remoteCancellationRequested = true;
                    }
                    else {
